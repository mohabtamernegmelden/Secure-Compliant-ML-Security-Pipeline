...
At boot time, the backend dynamically loads column configurations, scaling assets, and encoding assets to prepare the feature vector on-demand. If no model configuration is present, prediction endpoints return a clean `503 Service Unavailable` status code to prevent execution failure. You can swap in a new model by replacing the binary files and updating a single JSON configuration.

---

## 2. Required ML Artifacts
The backend loads all ML assets from the project's `models/` directory. When integrating the official model, place the following files inside the `models/` folder:

```text
models/
├── best_model.pkl           # [REQUIRED] The serialized classifier model object
├── scaler.pkl               # [OPTIONAL] Fitted scikit-learn scaler object
├── encoder.pkl              # [OPTIONAL] Fitted scikit-learn encoder object
...
* The prediction schema must match the output format of the official model:
  * **Binary classification (default)**: If the model outputs binary integers (`0` / `1`), keep the current prediction schema.
  * **Multi-class / String labels**: If the model outputs string categories (e.g. network intrusion labels like `BENIGN`, `PORTSCAN`) or multi-class indices, you must update the response schema (`PredictionResponse.prediction` to `str` or `int` appropriately) in `app/schemas/prediction.py`.
...
The `PreprocessingService` automatically coordinates scaling and encoding based on the contents of the config file.
...
Copy the new trained artifacts directly into the `models/` directory, overwriting the existing files:
```bash
...
Open `app/schemas/prediction.py` and update `PredictionInput` to declare all fields required by your model. Use appropriate Pydantic Types and range constraints (e.g. `ge` or `le`):
```python
...
```
*Note: The backend developer must update the PredictionInput schema after receiving the official ML feature list.*
...
* **`app/schemas/prediction.py`**: Update `PredictionInput` to declare your model's input features, and adjust `PredictionResponse.prediction` type if the classification target format changed (e.g. to `str` for string outputs).

### Advanced Customizations:
* **Custom Preprocessing Logic**: If your pipeline requires custom column transformations (e.g., custom date conversions or mathematical log transforms) instead of standard scikit-learn scaling, edit the `preprocess_batch` method in `app/services/preprocessing_service.py`.
* **Direct Model Properties**: If you need to custom-tune the model parameters (e.g. manually overriding a classification probability threshold), edit the `predict` method in `app/services/model_service.py`.
...
- [ ] **Probability Output**: Probability matches the soft confidence probability of the predicted class.
- [ ] **Health Endpoint Green**: `GET /health` returns `status: "healthy"` and `model_loaded: true`.
- [ ] **All Tests Pass**: Pytest reports all tests passing.
...
2. If your model uses raw pandas DataFrames with categorical categories, you will need to adjust the `PreprocessingService` or `ModelService` to construct a DataFrame instead of a raw NumPy array:
   * Edit `model_service.py` `predict` method:
     ...
     import pandas as pd
     # ... inside predict()
     df_features = pd.DataFrame(features, columns=self.num_features + self.cat_features)
