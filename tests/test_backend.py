...
def test_model_artifacts_are_present():
    required_files = [
    ...
    assert not missing, f"Missing production model artifacts: {missing}"
...
def test_ui_endpoint_serves_frontend():
    response = client.get("/ui")
    assert response.status_code == 200
    assert "Secure & Compliant ML Security Pipeline" in response.text
